...
int main(int argc, char *argv[])
{
  ...
  const CBLAS_ORDER order = (Order == CblasColMajor) ? CblasColMajor : CblasRowMajor;
  const CBLAS_TRANSPOSE transa = (TransA == CblasNoTrans) ? CblasNoTrans : CblasTrans;
  const CBLAS_TRANSPOSE transb = (TransB == CblasNoTrans) ? CblasNoTrans : CblasTrans;
