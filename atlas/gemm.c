...
#define TESTBLAS_GEMM cblas_dgemm
#define CBLAS_GEMM cblas_dgemm
#else
typedef float precision;
#define TESTBLAS_GEMM cblas_sgemm
...
int main(int argc, char *argv[])
{
  ...
  const enum CBLAS_ORDER order = (Order == CblasColMajor) ? CblasColMajor : CblasRowMajor;
  const enum CBLAS_TRANSPOSE transa = (TransA == CblasNoTrans) ? CblasNoTrans : CblasTrans;
  const enum CBLAS_TRANSPOSE transb = (TransB == CblasNoTrans) ? CblasNoTrans : CblasTrans;
